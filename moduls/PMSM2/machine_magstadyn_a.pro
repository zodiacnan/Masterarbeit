...
PostProcessing {
  ...
  { Name MagStaDyn_a_2D ; NameOfFormulation MagStaDyn_a_2D ;
   PostQuantity {
     ...
     { Name Inductance_from_Flux ; Value { Term { Type Global; [ $Flux_a * 1e3/II ] ; In DomainDummy ; } } }
      { Name Inductance_from_MagEnergy ; Value { Term { Type Global; [ 2 * $MagEnergy * 1e3/(II*II) ] ; In DomainDummy ; } } }
      { Name Inductance_Fluxq ; Value { Term { Type Global; [ CompY[Flux_dq0[]] * 1e3/IQ ] ; In DomainDummy ; } } }
...
PostOperation Get_LocalFields UsingPost MagStaDyn_a_2D {
  ...
  Print[ js, OnElementsOf DomainS,
	 ...
	 AppendTimeStepToFileName Flag_SaveAllSteps ] ;
  Print[ nb,  OnElementsOf Domain,
	 File StrCat[ResDir,"nb",ExtGmsh], LastTimeStepOnly,
	 AppendTimeStepToFileName Flag_SaveAllSteps] ;
  Print[ b,  OnElementsOf Domain,
	 File StrCat[ResDir,"b",ExtGmsh], LastTimeStepOnly,
	 AppendTimeStepToFileName Flag_SaveAllSteps] ;
  Print[ b_tangent,  OnElementsOf Domain,
	 File StrCat[ResDir,"b_tangent",ExtGmsh], LastTimeStepOnly,
	 AppendTimeStepToFileName Flag_SaveAllSteps] ;
  Print[ b_radial,  OnElementsOf Domain,
	 File StrCat[ResDir,"b_radial",ExtGmsh], LastTimeStepOnly,
	 AppendTimeStepToFileName Flag_SaveAllSteps] ;
  ...
  Print[ az, OnElementsOf Domain,
	 File StrCat[ResDir,"az",ExtGmsh], LastTimeStepOnly,
	 AppendTimeStepToFileName Flag_SaveAllSteps ] ;
}
...
PostOperation Get_GlobalQuantities UsingPost MagStaDyn_a_2D {
  ...
  Print[ MagEnergy[Domain], OnGlobal, Format TimeTable,
    File > StrCat[ResDir,"ME",ExtGnuplot], LastTimeStepOnly, StoreInVariable $MagEnergy,
    SendToServer StrCat[po,"41Magnetic Energy [W]"],  Color "LightYellow" ];

  Print[ Inductance_from_Flux, OnRegion DomainDummy, Format Table, LastTimeStepOnly, File StrCat[ResDir,"Inductance",ExtGnuplot],
    SendToServer StrCat[po,"50Inductance from Flux [mH]"], Color "LightYellow" ];
 Print[ Inductance_Fluxq, OnRegion DomainDummy, Format Table, LastTimeStepOnly, File StrCat[ResDir,"Inductance",ExtGnuplot],
    SendToServer StrCat[po,"50Lq [mH]"], Color "LightYellow" ];

  Print[ Inductance_from_MagEnergy, OnRegion DomainDummy, Format Table, LastTimeStepOnly, File StrCat[ResDir,"InductanceM",ExtGnuplot],
    SendToServer StrCat[po,"51Inductance from Magnetic Energy [mH]"], Color "LightYellow" ];
  Print[ S, OnRegion DomainDummy, Format Table, LastTimeStepOnly, File StrCat[ResDir,"S",ExtGnuplot],
    SendToServer StrCat[po,"ComplexPowe [W]"], Color "LightYellow" ];
...
PostOperation Get_Torque UsingPost MagStaDyn_a_2D {
  ...
  Print[ Torque_vw[Rotor_Airgap], OnGlobal, Format TimeTable,
    File > StrCat[ResDir,"Ts",ExtGnuplot], LastTimeStepOnly, StoreInVariable $Tstator,
	 SendToServer StrCat[po_mecT, "T_vw"]{0}, Color "Ivory" ];
