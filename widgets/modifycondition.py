...
class add_bc(QWidget):
    ...
    def GUI(self):
        ...
        self.nameCombo_list = [self.tr('A = 0'), self.tr('A = Const'), self.tr('A = 𝐴𝑘'), self.tr('A = -𝐴𝑘')]
        self.nameCombo.addItems(self.nameCombo_list)
        self.nameCombo.setCurrentIndex(0)
        self.nameCombo.currentIndexChanged.connect(self.Group_2)
    ...
    def Group_2(self):
        nr = self.nameCombo.currentIndex()
        if nr == 0:
            currentText = 'Dirichlet Boudary'
        if nr == 1:
            currentText = 'Neumann Boudary'
        if nr == 2:
            currentText = 'Positiv Perodic Condition'
        if nr == 3:
            currentText = 'Negativ Perodic Condition'
        self.tip.setText(currentText)
